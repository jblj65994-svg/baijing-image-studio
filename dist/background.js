...
const templates = [
  ['darkGraphitePhoto', '暗灰岩面'],
  ['warmLimestonePhoto', '暖灰石灰岩'],
  ['champagneTravertinePhoto', '香槟洞石'],
  ['smokedSlatePhoto', '烟熏板岩'],
  ['greenGrayPhoto', '绿灰石面'],
  ['ivoryMarblePhoto', '象牙大理石'],
];

const backdropPresets = {
  darkGraphitePhoto: ['#717a73', '#454b47', '#222725', 31, 1.15],
  warmLimestonePhoto: ['#e7ddca', '#b6ab98', '#817766', 32, 0.88],
  champagneTravertinePhoto: ['#f0d2a6', '#c69a61', '#8d6437', 33, 0.9],
  smokedSlatePhoto: ['#56615d', '#303836', '#101413', 34, 1.2],
  greenGrayPhoto: ['#9cad9f', '#657d70', '#2e4239', 35, 0.95],
  ivoryMarblePhoto: ['#faf5ea', '#ded6c7', '#bbb2a3', 36, 0.72],
};
...
function drawLowAngleBackdrop(targetCtx, width, height, top, mid, floor, seed, contrast = 1) {
  const horizon = Math.floor(height * 0.42);
  let gradient = targetCtx.createLinearGradient(0, 0, 0, horizon);
  gradient.addColorStop(0, top);
  gradient.addColorStop(1, mid);
  targetCtx.fillStyle = gradient;
  targetCtx.fillRect(0, 0, width, horizon);

  gradient = targetCtx.createLinearGradient(0, horizon, width, height);
  gradient.addColorStop(0, mid);
  gradient.addColorStop(1, floor);
  targetCtx.fillStyle = gradient;
  targetCtx.fillRect(0, horizon, width, height - horizon);

  for (let i = 0; i < 780; i += 1) {
    const x = noise(i, 1, seed) * width;
    const y = noise(i, 2, seed) * height;
    ...
    targetCtx.rotate((noise(i, 6, seed) - 0.5) * 0.8);
    targetCtx.fillStyle = noise(i, 7, seed) > 0.48 ? `rgba(255,255,255,${alpha})` : `rgba(0,0,0,${alpha})`;
    targetCtx.beginPath();
  ...
  for (let i = 0; i < 140; i += 1) {
    const x = -80 + noise(i, 11, seed) * (width + 160);
    const y = horizon + noise(i, 12, seed) * (height - horizon);
    const length = 35 + noise(i, 13, seed) * 220;
    const angle = (noise(i, 14, seed) - 0.5) * 0.22;
    const alpha = 0.035 + noise(i, 15, seed) * 0.08;
    targetCtx.strokeStyle = noise(i, 16, seed) > 0.45 ? `rgba(255,255,255,${alpha})` : `rgba(0,0,0,${alpha})`;
    targetCtx.lineWidth = 0.7 + noise(i, 17, seed) * 1.5;
    targetCtx.beginPath();
  ...
  let seam = targetCtx.createLinearGradient(0, horizon - 18, 0, horizon + 90);
  ...
  targetCtx.fillRect(0, horizon - 18, width, 108);

  const spotlight = targetCtx.createRadialGradient(width * 0.52, height * 0.66, 10, width * 0.52, height * 0.66, width * 0.42);
  spotlight.addColorStop(0, 'rgba(255,255,255,.16)');
  spotlight.addColorStop(1, 'rgba(255,255,255,0)');
  targetCtx.fillStyle = spotlight;
  targetCtx.fillRect(0, 0, width, height);

  const vignette = targetCtx.createRadialGradient(width * 0.5, height * 0.56, width * 0.26, width * 0.5, height * 0.56, width * 0.78);
...
function makeTemplatePreview(value) {
  const thumb = document.createElement('canvas');
  thumb.width = 360;
  thumb.height = 220;
  const thumbCtx = thumb.getContext('2d');
  const preset = backdropPresets[value];
  if (preset) drawLowAngleBackdrop(thumbCtx, thumb.width, thumb.height, ...preset);

  const focus = thumbCtx.createRadialGradient(thumb.width * 0.5, thumb.height * 0.7, 8, thumb.width * 0.5, thumb.height * 0.7, thumb.width * 0.34);
  focus.addColorStop(0, 'rgba(255,255,255,.16)');
  focus.addColorStop(1, 'rgba(255,255,255,0)');
  thumbCtx.fillStyle = focus;
...
function drawTemplate() {
  ...
  if (backdropPresets[template]) drawLowAngleBackdrop(ctx, canvas.width, canvas.height, ...backdropPresets[template]);
  else {
    ctx.fillStyle = '#fff';
...
$('#templates').innerHTML = templates.map(([value, label], index) => (
  `<button class="template asset${index === 0 ? ' selected' : ''}" data-template="${value}" type="button" style="background-image:url('${makeTemplatePreview(value)}')"><span>${label}</span></button>`
)).join('');
